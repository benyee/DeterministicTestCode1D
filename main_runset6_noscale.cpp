...
int main ()
{
    ...
    input->setfileName("defaultinput6.txt");
    ...
    static const double alpha_arr[] = {30};
        ...
        for(int j = dx.size()-1; j>=0;j--){
            for(unsigned int m = 0; m<alpha_mode.size();m++){
                for(int i = accel_size-1; i>=0;i--){
                    ...
                    ostringstream ss;
                    if(alpha_mode[m] == 0){
                        ss<<"OutputFiles/Run"<<runnum<<"/output_"<<runnum<<"_DD_"<<k<<"_"<<j<<"_"<<i<<"_.txt";
                    }else if (alpha_mode[m] ==30){
                        ss<<"OutputFiles/Run"<<runnum<<"/output_"<<runnum<<"_new_"<<k<<"_"<<j<<"_"<<i<<"_.txt";
                    }else if (alpha_mode[m] ==2){
                        ss<<"OutputFiles/Run"<<runnum<<"/output_"<<runnum<<"_SC_"<<k<<"_"<<j<<"_"<<i<<"_.txt";
                    }else{
                        ss<<"OutputFiles/Run"<<runnum<<"/output_"<<runnum<<"_"<<alpha_mode[m]<<"_"<<k<<"_"<<j<<"_"<<i<<"_.txt";
                    }
...
//                  cout<<"Finished setting parameters..."<<endl;
                    ...
                    if(writeToFile){
                        ...
                        if(input_run->get_it_num()>0 && !solutionFound){
                            ...
                            if(alpha_mode[m] == 0){
                                new_ss<<"OutputFiles/Run"<<runnum<<"/output_soln_"<<runnum<<"_DD_"<<k<<"_"<<j<<"_"<<0<<"_.txt";
                            }else if (alpha_mode[m] ==30){
                                new_ss<<"OutputFiles/Run"<<runnum<<"/output_soln_"<<runnum<<"_new_"<<k<<"_"<<j<<"_"<<0<<"_.txt";
                            }else if (alpha_mode[m] ==2){
                                new_ss<<"OutputFiles/Run"<<runnum<<"/output_soln_"<<runnum<<"_SC_"<<k<<"_"<<j<<"_"<<0<<"_.txt";
                            }else{
                                new_ss<<"OutputFiles/Run"<<runnum<<"/output_soln_"<<runnum<<"_"<<alpha_mode[m]<<"_"<<k<<"_"<<j<<"_0_.txt";
                            }
